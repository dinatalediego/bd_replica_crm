...
SOURCE_TO_TARGET = {
    ...
    "tipologia_ubicacion": "tipologia_ubicacion",
...
def _start_control_run(database_url: str, *, source_file: str, source_sha256: str, source_row_count: int, schema_name: str, table_name: str, delimiter: str, encoding: str, source_run_id: str) -> int | None:
    control_conn = None
    try:
        ...
        with control_conn.cursor() as cur:
            ...
            cur.execute(
                ...
                RETURNING run_id
                """,
                (source_file, source_sha256, source_row_count, schema_name, table_name, json.dumps({"delimiter": delimiter, "encoding": encoding, "source_run_id": source_run_id})),
            )
...
def _finish_control_run(database_url: str, run_id: int | None, *, status: str, loaded_row_count: int | None = None, rejected_row_count: int | None = None, snapshot_table: str | None = None, error_message: str | None = None) -> None:
    if run_id is None:
        return
    control_conn = None
    try:
        ...
        with control_conn.cursor() as cur:
            cur.execute(
                ...
                UPDATE etl_control.raw_mercado_load_runs
                   SET finished_at = now(), loaded_row_count = COALESCE(%s, loaded_row_count),
                       rejected_row_count = COALESCE(%s, rejected_row_count), snapshot_table = COALESCE(%s, snapshot_table),
                       status = %s, error_message = %s
                 WHERE run_id = %s
                """,
                (loaded_row_count, rejected_row_count, snapshot_table, status, error_message, run_id),
            )
    except psycopg2.Error:
        pass
    finally:
...
def _get_target_columns(cur, schema_name: str, table_name: str) -> list[str]:
    ...
    if cur.fetchone()[0] is None:
        raise RuntimeError(f"La tabla destino {schema_name}.{table_name} no existe. Este loader requiere el contrato canónico ya creado.")
    ...
    if missing:
        raise RuntimeError(f"El contrato de {schema_name}.{table_name} no contiene columnas obligatorias: {', '.join(missing)}")
    return columns
...
def _build_canonical_rows(rows: list[dict[str, str]], target_columns: list[str], source_run_id: str) -> tuple[list[str], list[tuple]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        missing_required = [col for col in ("codigo", "codigo_proyecto", "id") if canonical.get(col) in (None, "")]
        if missing_required:
...
def load_raw_mercado(database_url: str, file_path: str, *, schema_name: str = "raw_mercado", table_name: str = "unidades", delimiter: str = ",", encoding: str = "utf-8-sig", snapshot: bool = True, replace: bool = True) -> MercadoLoadResult:
    ...
    run_id = _start_control_run(database_url, source_file=str(path), source_sha256=source_hash, source_row_count=len(rows), schema_name=schema_name, table_name=table_name, delimiter=delimiter, encoding=encoding, source_run_id=source_run_id)
    ...
    try:
        with conn.cursor() as cur:
            ...
            if snapshot:
                ...
                cur.execute(sql.SQL("CREATE TABLE {}.{} AS TABLE {}.{}").format(sql.Identifier(schema_name), sql.Identifier(snapshot_table), sql.Identifier(schema_name), sql.Identifier(table_name)))
            if replace:
                cur.execute(sql.SQL("TRUNCATE TABLE {}.{}").format(sql.Identifier(schema_name), sql.Identifier(table_name)))
            query = sql.SQL("INSERT INTO {}.{} ({}) VALUES ({})").format(sql.Identifier(schema_name), sql.Identifier(table_name), sql.SQL(", ").join(map(sql.Identifier, insert_columns)), sql.SQL(", ").join(sql.Placeholder() * len(insert_columns)))
            cur.executemany(query, payload)
            cur.execute(sql.SQL("SELECT count(*), count(DISTINCT codigo), count(DISTINCT _etl_source_run_id) FROM {}.{} WHERE _etl_source_run_id = %s::uuid").format(sql.Identifier(schema_name), sql.Identifier(table_name)), (source_run_id,))
            loaded, distinct_codes, distinct_runs = map(int, cur.fetchone())
            if loaded != len(rows):
                raise RuntimeError(f"QA falló: se leyeron {len(rows)} filas pero se cargaron {loaded}.")
            if distinct_codes != len(rows):
                raise RuntimeError(f"QA falló: {loaded} filas cargadas pero solo {distinct_codes} códigos distintos.")
            if distinct_runs != 1:
                raise RuntimeError("QA falló: la carga no quedó asociada a un único _etl_source_run_id.")
        conn.commit()
        _finish_control_run(database_url, run_id, status="success", loaded_row_count=loaded, rejected_row_count=0, snapshot_table=snapshot_table)
        return MercadoLoadResult(run_id=run_id, source_file=str(path), source_sha256=source_hash, rows_read=len(rows), rows_loaded=loaded, rows_rejected=0, snapshot_table=snapshot_table, status="success")
    except Exception as exc:
        conn.rollback()
        _finish_control_run(database_url, run_id, status="failed", loaded_row_count=0, error_message=str(exc))
        raise
    finally:
        conn.close()
